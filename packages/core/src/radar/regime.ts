 ...
 * - restrictivo: 10 años ≥ 4,5% o subió ≥ 40 pb en 3 meses → penalidad a lo sensible a tasas.
 * - expansivo: bajó ≥ 40 pb en 3 meses y está por debajo de 4%.
 * - neutral: el resto.
 *
 * NO genera una reserva en efectivo. Lo intenté el 2026-09-10 y estaba mal por tres motivos: una reserva sin
 * regla de despliegue es plata muerta que se acumula sola; su retorno (SGOV ≈ 3,4% anual) pierde contra el núcleo
 * (VTI ≈ 17,7% a 12 meses); y con la cartera al 0% de núcleo, lo que baja el riesgo es diversificar, no esperar.
 * Si se quiere amortiguar el momento de entrada, el plan sugiere escalonar la compra (ver `tranches`).
 */
...
export function assessRegime(tnx: Candle[]): MacroRegime | null {
  ...
  return { state, asOf: last.date, tenYearPct, change3mBp, why };
}
