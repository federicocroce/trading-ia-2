...
export function qualityBar(f: FundamentalsInput, q: RadarPolicy["quality"]): { ok: boolean; reason?: string; mcapUsd: number | null; dollarVolumeUsd: number | null } {
  const mcap = mcapUsd(f.profile.shareOutstanding, f.priceUsd);
  const vol = dollarVolumeUsd(f.metrics["3MonthAverageTradingVolume"], f.priceUsd);
  if (f.priceUsd < q.minPrice) return { ok: false, reason: `precio ${f.priceUsd} < ${q.minPrice}`, mcapUsd: mcap, dollarVolumeUsd: vol };
  if (mcap === null) return { ok: false, reason: "sin acciones en circulación", mcapUsd: null, dollarVolumeUsd: vol };
  if (vol === null) return { ok: false, reason: "sin volumen de 3 meses", mcapUsd: mcap, dollarVolumeUsd: null };
  if (mcap < q.minMcapUsd) return { ok: false, reason: `capitalización ${Math.round(mcap / 1e6)}M < ${q.minMcapUsd / 1e6}M`, mcapUsd: mcap, dollarVolumeUsd: vol };
