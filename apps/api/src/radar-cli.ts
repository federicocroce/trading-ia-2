import { buildContributionPlan, measureRadar, rankRadar, refreshArgentina, refreshRadar, scanUniverse, withUsageStep } from "@thesis/pipeline";
...
const STEP: Record<string, string> = { scan: "scan", rank: "scan", refresh: "radar", measure: "radar", plan: "plan", argentina: "argentina" };
let code = 0;
await withUsageStep({ step: STEP[cmd ?? ""] ?? "cli" }, async () => {
  ...
  else { console.error("uso: tsx src/radar-cli.ts scan | rank | refresh | plan | measure | argentina"); code = 1; }
});
