...
- **Verificación de candidatos**: la ganancia núcleo se calcula desde los estados trimestrales de la SEC (XBRL), no del TTM de Finnhub — P/E, ROE y márgenes se recalculan sobre lo operativo sin extraordinarios, con la bandera `resultado_extraordinario` cuando el desvío supera el 25% (Finnhub queda al lado, sin pisarse). Eventos materiales de los últimos 90 días salen de noticias clasificadas por el modelo: uno grave manda a OBSERVAR por 90 días; uno moderado queda como salvedad sin cambiar el veredicto. Objetivos y calificaciones de analistas se extraen de titulares (Benzinga, TheFly). Las velas usadas para entrada, stop y cierre son siempre de sesiones cerradas (nunca la vela en curso de un refresco corrido en horario de mercado). Corre con los mismos `pnpm radar:rank` / `radar:refresh` de abajo. Diseño en [`docs/superpowers/specs/2026-09-09-radar-verificacion-design.md`](docs/superpowers/specs/2026-09-09-radar-verificacion-design.md).
...
**Ponerme al día.** Si la máquina estaba apagada o dormida a esa hora, no hace falta acordarse de nada: la API chequea un minuto después de arrancar y cada 30 minutos qué pasos quedaron sin correr (según `job_runs`, la última fecha cubierta por cada paso, con lo que ya hay en la base como respaldo) y corre solo esos, en orden: barrido y ranking (en segundo plano), Cartera, Radar, Argentina, plan, tesis. Lo ya hecho no se repite. El encabezado muestra la hora de la **última corrida** con un punto de estado (verde al día, ámbar pendiente, rojo con error); click abre el panel de corridas: cada paso con cuándo corre solo, su última corrida buena, el resumen, el último error (los errores quedan registrados en `job_runs` y el paso sigue pendiente) y un botón **Correr** por paso (`POST /catchup/run/:step`). El botón **Ponerme al día** corre solo lo pendiente; `CATCHUP_AUTO=0` apaga el chequeo automático. Rutas: `GET /catchup` (estado) y `POST /catchup` (correr).
