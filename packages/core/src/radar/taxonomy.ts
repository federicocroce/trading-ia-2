...
export const RadarPolicySchema = z.object({
  ...
  candidates: z.object({ top: z.number(), preselect: z.number(), chronicWeeks: z.number(), verifyPerRun: z.number().optional() }),
  contribution: z.object({ monthlyUsd: z.number(), coreTargetPct: z.number(), maxPositionPct: z.number(), maxNewPositionsPerMonth: z.number(), maxLinePctOfContribution: z.number(), coreSharePctWhileBelowTarget: z.number().optional(), sumarSharePctOfRest: z.number().optional(), watchLinesMax: z.number().optional(), etfLinesMax: z.number().optional(), reservePctWhenRestrictive: z.number().optional(), reserveSymbol: z.string().optional() }),
});
