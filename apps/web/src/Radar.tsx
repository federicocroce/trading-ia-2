...
const QUE_CAMBIO: Record<PlanChange["change"], string> = { entra: "entra", sale: "sale", monto: "cambia", aviso: "aviso" };
...
function PlanCard({ p, top, radarDate, onBuild, busy }: { p: ContributionPlan; top: RadarTop | null; radarDate: string | null; onBuild: (amountUsd: number) => Promise<void>; busy: boolean }) {
  ...
  return (
    <div className="card" style={{ overflowX: "auto" }}>
      ...
      <div className="muted" style={{ marginTop: 6, fontSize: 12 }}>Cómo se arma: el núcleo recibe una parte fija mientras esté bajo su objetivo; después se suma a lo que Cartera propone, y lo que queda va a las compras nuevas por convicción, repartidas según la convicción, más una de tu seguimiento. Una acción entra si pasa las reglas fijas (stop fuera del ruido, que diversifique, sin subas extremas, y que Cartera no diga REVISAR o VENDER si ya la tenés). De la IA solo la deja afuera un "evitar" de la verificación web; lo demás ("con reservas", "pendiente", y todo lo que diga la revisión antes de comprar, objeciones incluidas) va escrito con ⚠ en su línea y decidís vos. Si queda afuera, su lugar va al núcleo y el motivo está en "Candidatas que hoy NO se compran". Antes de ejecutar mirá "cuándo entrar": "esperar" con orden limitada es un retroceso al precio indicado; "si cierra arriba de" es una compra solo si cierra arriba de ese nivel. Ninguna de las dos va a mercado. Cargá las operaciones en Cartera cuando las hagas.</div>
    </div>
  );
}
