...
export async function carteraCurve(store: Pick<CarteraStore, "transactions" | "positions"> & Pick<RadarStore, "candles">): Promise<CurveResponse> {
  ...
  const flows = transactions.filter((t) => t.type === "BUY" || t.type === "SELL");
  if (!flows.length) return { curve: null, error: null, computedAt };
  const first = flows.map((t) => t.date).sort()[0]!;
  const symbols = [...new Set(flows.map((t) => t.symbol.toUpperCase()))];
