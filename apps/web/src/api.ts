...
export interface MacroRegime { state: "restrictivo" | "neutral" | "expansivo"; asOf: string; tenYearPct: number; change3mBp: number | null; reservePct: number; why: string }
export interface RadarTop { date: string | null; overweight: Record<string, number>; regime?: MacroRegime | null; picks: TopPick[] }
export interface PlanLine { symbol: string; kind: "reserva" | "nucleo" | "sumar" | "comprar" | "seguimiento"; amountUsd: number; rationale: string; close: number | null; alpha30dPct: number | null; alpha90dPct: number | null; entryHigh?: number | null; stop?: number | null; target?: number | null; ret12mPct?: number | null; priority?: number | null }
