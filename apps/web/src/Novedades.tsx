...
import { api, type Novedades as NovedadesData, type Verb } from "./api";
...
export function Novedades() {
  ...
  return (
    <div className="card">
      ...
      <Sec title="Veredictos de tu cartera que cambiaron" items={n.verdictChanges.map((v) => <li key={v.symbol}><SymbolLink symbol={v.symbol} /> {v.from} → <CarteraVerdict symbol={v.symbol} verb={v.to as Verb} plan={plan} /> <span className="muted">{v.reason}</span></li>)} />
      ...
      {/* Hasta el 14/9 esto se llamaba "Entran a COMPRAR" y listaba HSBC y ORRF, que el plan no compraba: el dueño lo
          leyó como una orden. Son candidatas nuevas del Radar; COMPRAR solo aparece si el plan de hoy las compra. */}
      {/* 15/9: el seguimiento también cuenta, y lo que ya tenés se dice (TSM figuraba como "nueva" sin aclarar que es tuya). */}
      <Sec title="Nuevas en el Radar (pasan los filtros)" items={n.enteredBuy.map((c) => <li key={c.symbol}><SymbolLink symbol={c.symbol} /> <RadarVerdict symbol={c.symbol} verdict="COMPRAR" plan={plan} /> <span className="muted" title="Score contra sus pares en desviaciones típicas, no un porcentaje.">{c.kind === "etf" ? "· ETF" : c.kind === "watch" ? "· de tu seguimiento" : `· score ${c.score?.toFixed(2) ?? "—"} vs pares`}</span>{c.held && <span className="muted"> · ya la tenés</span>}</li>)} />
