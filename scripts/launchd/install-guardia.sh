#!/bin/zsh
# Instala solo la guardia de la mañana (23/9): lunes a viernes 10:00, después del refresco (07:50) y del agente
# de verificación (08:15). No toca la API, el web ni los otros agentes. Correr desde el repo principal.
set -e
...
DIAS=""
for d in 1 2 3 4 5; do DIAS="$DIAS<dict><key>Weekday</key><integer>$d</integer><key>Hour</key><integer>10</integer><key>Minute</key><integer>0</integer></dict>"; done
cat > "$PLIST" <<PL
...
echo "instalado com.thesis-engine.guardia (lunes a viernes 10:00, log en $LOGS/guardia.log)"
