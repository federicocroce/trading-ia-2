...
export const HELP: Record<string, { label: string; short: string }> = {
  ...
  entrada: { label: "entrada", short: "Cuándo y a qué precio. Si está en zona, la franja va del cierre a +2%. Si está estirada, no es hoy: dice el precio a esperar (la media de 20 ruedas) y cuántas ruedas vale esa orden. Si el precio ya se escapó por arriba, no lo corras." },
  stop: { label: "stop", short: "Si cierra por debajo, se vende. Para una compra nueva: el más bajo entre el stop de seguimiento (máximo de 22 ruedas menos 3 ATR) y el piso de la franja menos 2,5 ATR, para que el ruido de un día no lo ejecute. Si ya lo tenés en cartera, es el de tu posición. El % es lo que perdés desde el precio actual." },
  objetivo: { label: "objetivo", short: "Precio donde la operación paga 2 veces lo que arriesga hasta el stop. NO es una ganancia esperada ni un pronóstico: es aritmética sobre el stop, así que sube y baja con la volatilidad y no con la empresa. Medido sobre los 26 COMPRAR del 11/9, la correlación entre el % al objetivo y la distancia al stop es 1,0000." },
  ...
  tamano: { label: "tamaño", short: "Acciones y dólares para arriesgar 1% de la cartera hasta el stop, con tope del 10% de la cartera por posición. Un papel volátil tiene stop más lejos y por eso menos acciones. Los dólares están calculados al TECHO DE LA FRANJA DE COMPRA, que es lo que vas a pagar, no al precio de hoy: por eso el monto no da si multiplicás las acciones por el precio de la columna de al lado. El precio usado va escrito en la misma celda." },
  ...
  cuandoEntrar: { label: "cuándo entrar", short: "Si hoy es el día, y a qué precio. 'comprar ahora' con la franja de compra. 'esperar' con el precio a esperar: la media de 20 ruedas si está muy estirada, o el máximo de las últimas 10 si cayó bajo su media de 50. La condición vale 15 ruedas; pasadas, se vuelve a evaluar. El núcleo va siempre a mercado." },
  stopPlan: { label: "stop", short: "Si cierra por debajo, se vende: la tesis se anuló. En una compra nueva queda al menos 2,5 ATR debajo del piso de la franja; en un SUMAR es el de tu posición. El % es lo que perdés desde el precio de hoy. No tiene plazo." },
