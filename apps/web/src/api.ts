...
export interface PlanLine { symbol: string; kind: "nucleo" | "sumar" | "comprar" | "seguimiento"; amountUsd: number; rationale: string; /** Avisos de la verificación web y de la revisión antes de comprar: no frenan, se muestran al lado de COMPRAR (18/9). */ avisos?: string[]; close: number | null; alpha30dPct: number | null; alpha90dPct: number | null; entryHigh?: number | null; stop?: number | null; target?: number | null; ret12mPct?: number | null; ret12mPartial?: boolean | null; priority?: number | null; entry?: EntryTiming | null; /** Debajo de este precio la orden no se ejecuta: stop + 1 ATR. */ minPrice?: number | null; /** La base única de la orden (15/9): cantidad, riesgo y % se cuentan contra este precio. */ orderPrice?: number | null; qty?: number | null; trancheUsd?: number | null; trancheQty?: number | null; /** De qué rueda es `close`. */ closeDate?: string | null }
...
export interface PlanChange { symbol: string; change: "entra" | "sale" | "monto" | "aviso"; fromUsd: number; toUsd: number; source: "mercado" | "verificacion" | "regla" | "usuario" | "reparto" | "monto"; cause: string }
...
export type HechoTipo = "guia" | "ganancia_por_reservas" | "ganancia_extraordinaria" | "oferta_de_compra" | "investigacion_regulatoria" | "evento_de_capital";
...
export interface TickerPage {
  ...
  tags: Tags | null;
  fundamentals: { asOf: string; metrics: Record<string, number | null>; metricsRaw?: Record<string, number | null> | null; statementsAsOf?: string | null; peers: string[]; mcapUsd: number | null; dollarVolumeUsd: number; nextEarnings: string | null; insiderBuys90d: number | null; insiderSells90d: number | null; analyst: { strongBuy: number; buy: number; hold: number; sell: number; strongSell: number; period: string } | null; earningsSurprises: Array<{ period: string; actual?: number | null; estimate?: number | null; surprisePercent: number | null }> | null } | null;
  /**
