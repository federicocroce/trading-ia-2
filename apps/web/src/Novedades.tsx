...
export function Novedades() {
  ...
  return (
    <div className="card">
      ...
      <Sec title="Nuevas en el Radar (pasan los filtros)" items={n.enteredBuy.map((c) => <li key={c.symbol}><SymbolLink symbol={c.symbol} /> <RadarVerdict symbol={c.symbol} verdict="COMPRAR" plan={plan} /> <span className="muted" title="Score contra sus pares en desviaciones típicas, no un porcentaje.">{c.kind === "etf" ? "· ETF" : `· score ${c.score?.toFixed(2) ?? "—"} vs pares`}</span></li>)} />
