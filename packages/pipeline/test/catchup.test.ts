...
describe("dueSteps", () => {
  it("pendiente = última corrida anterior a la esperada; sin corrida nunca = pendiente; en orden de ejecución", () => {
    ...
    expect(due.map((d) => d.id)).toEqual(["radar", "argentina", "plan"]);
    expect(due[0]).toEqual({ id: "radar", label: stepById("radar").label, last: "2026-09-07", expected: "2026-09-08" });
    expect(due[1]?.last).toBeNull();
  });
  ...
  it("el barrido semanal se ejecuta antes que lo diario; el ranking no es un paso del calendario", () => {
    ...
    expect(due.map((d) => d.id)).toEqual(["scan", "cartera", "radar", "argentina", "plan", "tesis"]);
