...
export function Novedades() {
  ...
  return (
    <div className="card">
      ...
      <Sec title="Seguimiento para revisar" items={n.watchResolved.map((w) => <li key={w.symbol} className="warn"><SymbolLink symbol={w.symbol} /> {STATUS_LABEL[w.status] ?? w.status}{pct(w.returnPct)}</li>)} />
      <Sec title="Entran a COMPRAR" items={n.enteredBuy.map((c) => <li key={c.symbol} className="ok"><SymbolLink symbol={c.symbol} /> <span className="muted">{c.kind === "etf" ? "ETF" : `score ${c.score?.toFixed(2) ?? "—"}`}</span></li>)} />
      <Sec title="Salen de COMPRAR" items={n.leftBuy.map((c) => <li key={c.symbol} className="muted"><SymbolLink symbol={c.symbol} /> ahora {c.now}</li>)} />
      <Sec title="Tesis propuestas, esperan tu decisión" items={n.proposedTheses.slice(0, 6).map((t) => <li key={t.id}><SymbolLink symbol={t.ticker} /> {t.direction} · edge {(t.edge * 100).toFixed(0)}% <span className="muted">{t.summary}</span></li>)} />
